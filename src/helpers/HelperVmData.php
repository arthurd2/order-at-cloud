...
class HelperVmData
{
    ...
    static public function getVmsData($max = PHP_INT_MAX) {
        //TODO Clean this mess
        //if (!is_null(HelperVmData::$data)) return HelperVmData::$data;
        
        
        $cache = Cache::$cache->get('HelperVmData');
        if ($cache !== false) {
            //HelperVmData::$data = $cache;
            //return HelperVmData::$data;
        }
        ...
        $vmss = json_decode(file(FNVM) [0], true);
    ...
    static public function getRealCvmp($max = PHP_INT_MAX) {
        $cvmp = [];
        $data = HelperVmData::getVmsData($max);
